...
ISR(CAN_INT_vect)
{
	...
	mob >>= 4; // -> mob number 0..5
	if((CANSTMOB & ~0x20) ==0) //RX
	{
	...
	else if((CANSTMOB & ~0x40) ==0) //TX
	{
		...
		else //disable mob to stop interrupts.
		{
			CANSTMOB=0;   // reset INT reason
			CANCDMOB = 0x00;
		}
...
{
	if( mob >= NO_MOBS)         // illegal MOB number
		return TRUE;
	canlist[ (unsigned)mob]=callback;
