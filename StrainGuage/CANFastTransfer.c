...
{
	beginCANFast(ReceiveCAN, sizeof(ReceiveCAN), STRAIN_CAN_ADDRESS);
}
...
void checkCANFTdata(void)
{
	if(ReceiveDataCAN())
	{
//    setPerformNavigationCommand((int)receiveArrayAddressCAN[MOTOR_COMMAND_MODE_DATA_ADDRESS]);
//    setNavigationCommandData((int)receiveArrayAddressCAN[MOTOR_VELOCITY_DATA_ADDRESS]);
//    setPauseCommand((int)receiveArrayAddressCAN[MOTOR_PAUSE_COMMAND_ADDRESS]);
//    setPauseData((int)receiveArrayAddressCAN[MOTOR_PAUSE_DATA_ADDRESS]);
//    //Clear the array Values
//    receiveArrayAddressCAN[MOTOR_COMMAND_MODE_DATA_ADDRESS] = 0;
//    receiveArrayAddressCAN[MOTOR_VELOCITY_DATA_ADDRESS] = 0;
//    receiveArrayAddressCAN[MOTOR_PAUSE_COMMAND_ADDRESS] = 0;
//    receiveArrayAddressCAN[MOTOR_PAUSE_DATA_ADDRESS] = 0;


		//toggleLED(LED2);

		//setMotorCommandMode(receiveArrayAddressCAN[MOTOR_COMMAND_MODE_DATA_ADDRESS]);
		//if(getMotorCommandMode()==VELOCITY_MODE)
		//{
		//setMotorVelocity(receiveArrayAddressCAN[MOTOR_VELOCITY_DATA_ADDRESS]);
		//}
		//else
		//{
		//setMotorEncoderDistance(receiveArrayAddressCAN[MOTOR_ENCODER_DISTANCE_DATA_ADDRESS]);
		//
		//}
	}
}
...
void TransmitCANFast( CAN_packet *p, unsigned char mob) // interrupt callback
{
	...
	if(Transmit_buffer_GetCount(&TransmitBuffer) > 0) {
		...
		if(Transmit_buffer_GetCount(&TransmitBuffer)>6) {
			...
			p->id = ( address << 6) + moduleAddressCAN; //not passed through messages will have wrong sender address
